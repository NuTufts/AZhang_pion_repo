...
void Purity2D(){
  ...
  TH2F h("h","Purity vs. True Purity of Truth-Matched Pions;trackTruePurity (%);trackPurity (%)", 40,0,1,40,0,1); //create a histogram for reconstructed track energies
  ...
  Float_t trackTruePurity[100]; //the track reconstructed energy values will get assigned to this variable when looping through the tree. it is an array of floating point variables, with one entry for each track in the event. Choose a size for the array here (100) that will be larger than the actual array size for any particular event to make sure it is large enough to always accomodate all track values
  t -> SetBranchAddress("trackTruePurity", trackTruePurity); //since this variable is for an array, you don't need the "&" in the second argument, array variables just point to the memory address in C++
  Float_t trackPurity[100];
  t -> SetBranchAddress("trackPurity", trackPurity);
  ...
  for(Int_t i = 0; i < t->GetEntries(); ++i){
    ...
    for(Int_t j = 0; j < nTracks; ++j){
        if(trackPID[j] == 211 || trackTruePID[j] == 211){
            h.Fill(trackTruePurity[j], trackPurity[j]);
        }
    }
  }
...
//    c2->cd(4); h.ProfileY()->Draw();
  ...
  h.Draw("SAME PLC PMC COLZ EHIST"); //this should make a plot of the histogram pop up on your screen, but it doesn't seem to be working for me, so write it to a new output file as well:
  TFile f_out("Purity2D.root","RECREATE");
  h.Write();
