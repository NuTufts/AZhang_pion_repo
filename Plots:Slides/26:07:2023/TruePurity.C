...
void TruePurity(){
  ...
  TH1F h("h","TrackTruePurity for Truth-Matched Pions;trackTruePurity (%);# of Tracks", 40,0,1); //create a histogram for reconstructed track energies
  ...
  for(Int_t i = 0; i < t->GetEntries(); ++i){
    ...
    for(Int_t j = 0; j < nTracks; ++j){
        if(trackPID[j] == 211 || trackTruePID[j] == 211){
            h.Fill(trackTruePurity[j]);
            total += 1;
        }
        if (trackTruePurity[j] >= 0.9){
            highcompletion += 1;
        }
    }
  }
    cout << highcompletion << endl;
    cout << total << endl;
  ...
  TFile f_out("TruePurity.root","RECREATE");
  h.Write();
