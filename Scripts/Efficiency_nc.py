# Efficiency_nc.py
# Author: Andy Zhang
# Purpose: Find Reconstruction Efficiencies of True Primary Pions in nc Events ordered by Kinetic Energy, categorized by other Primary particles. Also organize improperly reconstructed pions by cause (Incorrect PDG, missing track, reconstructed as secondary).
...
h_simE_nc = rt.TH1F("h_simE_nc","Number of Reconstructed Pions by Energy (nc);Pion Kinetic Energy (MeV);# of Pions per 6.67e20 POT", 40,0,1200)

h_trueE_nc = rt.TH1F("h_trueE_nc","Number of Truth-Matched Pions by Energy (nc);Pion Kinetic Energy (MeV);# of Pions per 6.67e20 POT", 40,0,1200)

h_Eff_nc = rt.TH1F("h_Eff_nc","Pion Reconstruction Efficiency by Energy (nc);Pion Kinetic Energy (MeV);Reconstruction Efficiency", 40,0,1200)
...
h_simE_Mu = rt.TH1F("h_simE_Mu","Number of Reconstructed Pions by Energy (nc);Pion Kinetic Energy (MeV);# of Pions per 6.67e20 POT", 40,0,1200)

h_trueE_Mu = rt.TH1F("h_trueE_Mu","Number of Truth-Matched Pions by Energy (nc);Pion Kinetic Energy (MeV);# of Pions per 6.67e20 POT", 40,0,1200)

h_Eff_Mu = rt.TH1F("h_Eff_Mu","Pion Reconstruction Efficiency by Energy with Muons (nc);Pion Kinetic Energy (MeV);Reconstruction Efficiency", 40,0,1200)

h_recoComp_Mu = rt.TH1F("h_recoComp_Mu","Greatest Track Completeness of Truth-Matched Pion Events (nc);Reconstructed Track Completeness (%);# of Truth-Matched Pion Events", 20,0,1)

h_simE_Pr = rt.TH1F("h_simE_Pr","Number of Reconstructed Pions by Energy (nc);Pion Kinetic Energy (MeV);# of Pions per 6.67e20 POT", 40,0,1200)

h_trueE_Pr = rt.TH1F("h_trueE_Pr","Number of Truth-Matched Pions by Energy (nc);Pion Kinetic Energy (MeV);# of Pions per 6.67e20 POT", 40,0,1200)

h_Eff_Pr = rt.TH1F("h_Eff_Pr","Pion Reconstruction Efficiency by Energy with Protons (nc);Pion Kinetic Energy (MeV);Reconstruction Efficiency", 40,0,1200)

h_recoComp_Pr = rt.TH1F("h_recoComp_Pr","Greatest Track Completeness of Truth-Matched Pion Events (nc);Reconstructed Track Completeness (%);# of Truth-Matched Pion Events", 20,0,1)

h_simE_Neu = rt.TH1F("h_simE_Neu","Number of Reconstructed Pions by Energy (nc);Pion Kinetic Energy (MeV);# of Pions per 6.67e20 POT", 40,0,1200)

h_trueE_Neu = rt.TH1F("h_trueE_Neu","Number of Truth-Matched Pions by Energy (nc);Pion Kinetic Energy (MeV);# of Pions per 6.67e20 POT", 40,0,1200)

h_Eff_Neu = rt.TH1F("h_Eff_Neu","Pion Reconstruction Efficiency by Energy with Neutrons (nc);Pion Kinetic Energy (MeV);Reconstruction Efficiency", 40,0,1200)

h_recoComp_Neu = rt.TH1F("h_recoComp_Neu","Greatest Track Completeness of Truth-Matched Pion Events (nc);Reconstructed Track Completeness (%);# of Truth-Matched Pion Events", 20,0,1)

h_missing = rt.TH1F("h_missing","Improperly Reconstructed Pions with Missing Tracks by Energy (nc);Pion Kinetic Energy (MeV);# of Mis-Identified Tracks", 40,0,1200)

h_secondary = rt.TH1F("h_secondary","Improperly Reconstructed Pions Mis-Identified as their Secondaries by Energy (nc);Pion Kinetic Energy (MeV);# of Mis-Identified Tracks", 40,0,1200)

h_misID = rt.TH1F("h_misID","Improperly Reconstructed Pion Mis-Identified as Other Primary Particles by Energy (nc);Pion Kinetic Energy (MeV);# of Improperly Reconstructed Pions", 40,0,1200)

h_improper = rt.TH1F("h_improper","Total Improperly Reconstructed Pions by Energy (nc); Pion Kinetic Energy (MeV); # of Improperly Reconstructed Pions", 40,0,1200)

h_unconstructed_stack = rt.THStack("h_unconstructed_stack","Cause of Improper Pion Reconstruction by Energy (nc)")

nc = [] # List for event storage
...
for i in range(0, len(nc)):
    pion_index = len(nc[i]) - 1
    for j in range(0, pion_index):
        h_trueE_nc.Fill(nc[i][j].true_pionE, nc[i][j].xsecWeight)
        if(nc[i][pion_index][0] >= 1):
            h_trueE_Mu.Fill(nc[i][j].true_pionE, nc[i][j].xsecWeight)
            filtered[0] += 1
        if(nc[i][pion_index][1] >= 1):
            h_trueE_Pr.Fill(nc[i][j].true_pionE, nc[i][j].xsecWeight)
            filtered[1] += 1
        if(nc[i][pion_index][2] >= 1):
            h_trueE_Neu.Fill(nc[i][j].true_pionE, nc[i][j].xsecWeight)
            filtered[2] += 1
        if(nc[i][j].reco_found == True):
            h_recoComp_nc.Fill(nc[i][j].top_recoComp)
            h_simE_nc.Fill(nc[i][j].true_pionE, nc[i][j].xsecWeight)
            reco_fill_count += 1
            if(nc[i][pion_index][0] >= 1):
                h_simE_Mu.Fill(nc[i][j].true_pionE, nc[i][j].xsecWeight)
                filtered[3] += 1
                h_recoComp_Mu.Fill(nc[i][j].top_recoComp)
            if(nc[i][pion_index][1] >= 1):
                h_simE_Pr.Fill(nc[i][j].true_pionE, nc[i][j].xsecWeight)
                filtered[4] += 1
                h_recoComp_Pr.Fill(nc[i][j].top_recoComp)
            if(nc[i][pion_index][2] >= 1):
                h_simE_Neu.Fill(nc[i][j].true_pionE, nc[i][j].xsecWeight)
                filtered[5] += 1
                h_recoComp_Neu.Fill(nc[i][j].top_recoComp)
...
h_stack_nc = rt.THStack("h_stack_nc","Reconstruction of True Pions by Energy (nc)")
h_stack_Mu = rt.THStack("h_stack_Mu","Reconstruction of True Pions with Muons by Energy (nc)")
h_stack_Pr = rt.THStack("h_stack_Pr","Reconstruction of True Pions with Protons by Energy (nc)")
h_stack_Neu = rt.THStack("h_stack_Neu","Reconstruction of True Pions with Neutrons by Energy (nc)")

# Plot Beautification and Writing
f_out = rt.TFile("Pi_Efficiency_nc.root","RECREATE")
c_stack_nc = rt.TCanvas("c_stack_nc")
h_temp = rt.TH1F("h_temp","", 40,0,1200)
h_temp = h_simE_nc
h_temp.SetFillColor(rt.kGreen-3)
h_stack_nc.Add(h_temp)
h_temp = rt.TH1F("h_temp","", 40,0,1200)
h_temp.Add(h_trueE_nc, h_simE_nc, 1, -1)
h_temp.SetFillColor(rt.kRed-7)
legend = rt.TLegend(0.7, 0.1, 0.9, 0.3)
legend.AddEntry(h_simE_nc,"Good Reco","f")
...
c_Eff_nc = rt.TCanvas("c_Eff_nc")
h_Eff_nc.Draw()
c_Eff_nc.Update()
#c_Eff_nc.Draw("bar")
c_Eff_nc.Write()

c_recoComp_nc = rt.TCanvas("c_recoComp_nc")
h_recoComp_nc.Draw("bar")
c_recoComp_nc.Update()
#c_recoComp_nc.Draw("bar")
c_recoComp_nc.Write()
